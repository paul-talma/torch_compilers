...
class Tensor {
  public:
    ...
    //                  This is what you want when you need an independent
    //                  tensor.
    //
    ...
    // Move transfers the shared_ptr and leaves the source in a valid empty
    // state.
    //
    ...
    // No data is copied. The returned Tensor has different
  ...
  private:
    ...
    void print_rec(size_t dim,
                   std::vector<size_t> idx,
                   std::ostringstream &oss,
                   u32 max_elem_width) const;
    float max() const {
        float *base = storage_->data();
        float max = *base;
        u32 elems = numel();
        for (size_t id = 1; id < elems; ++id) {
            if (base[id] > max) {
                max = base[id];
            }
        }
